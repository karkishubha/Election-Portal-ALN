import { Image, Plus, Edit2, Trash2, Eye, EyeOff, Loader2, RefreshCw } from "lucide-react";
...
const AdminInfographics = () => {
  ...
  const { data, isLoading, isError, refetch } = useAdminInfographics(page);
  const createMutation = useCreateInfographic();
  const updateMutation = useUpdateInfographic();
  const deleteMutation = useDeleteInfographic();
  const togglePublishMutation = useToggleInfographicPublish();
  ...
  const handleAddResource = async () => {
    ...
    try {
      ...
      if (selectedFile) {
        const uploadResult = await uploadApi.uploadPdf(selectedFile, 'infographics');
          ...
          <div>
            <div className="flex items-center gap-3 mb-2">
              <div className="p-2 rounded-lg bg-section-education/20">
                <Image className="w-5 h-5 text-section-education" />
              </div>
            ...
            <p className="text-muted-foreground">
              Manage voter education infographics and related resources
            </p>
          </div>
          ...
          <div className="flex gap-2">
            ...
            <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
              ...
              <DialogContent className="bg-card">
                <DialogHeader>
                  <DialogTitle>Add New Infographic</DialogTitle>
                </DialogHeader>
                  ...
                  {resources.length === 0 ? (
                    <tr>
                      <td colSpan={3} className="p-8 text-center text-muted-foreground">
                        No infographics found. Add your first one!
                      </td>
                    </tr>
                  ) : (
